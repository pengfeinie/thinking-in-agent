...
def configure_retriever(uploaded_files):
    # 读取上传的文档，并写入一个临时目录
    docs = []
    temp_dir = tempfile.TemporaryDirectory(dir=r"/home/pfnie/sourcecodes")
    for file in uploaded_files:
        ...
        for doc in loaded_docs:
            print(f"File content: {doc.page_content}")
    ...
    if not valid_splits:
        raise ValueError("文档分割后没有有效的文本内容")

    # 使用OpenAI的向量模型生成文档的向量表示
    embeddings = OpenAIEmbeddings()
    vectordb = Chroma.from_documents(valid_splits, embeddings)
